...
const ChartcountOrdersByClientAddress = () => {
    ...
    const fetchOrdersByClientAddress = async () => {
        try {
          setLoading(true);
          const response = await axios.get('http://localhost:5001/Dashboard/countOrdersByClientAddressbysupplier', {
    ...
    }, [token]); // Effectue la requête à chaque changement du token
