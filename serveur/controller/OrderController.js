...
const createOrder = async (req, res) => {
  ...
  if (!supplierID) {
    return res
      .status(403)
      .json({ error: "Accès interdit : Utilisateur non authentifié" });
  }
  ...
  try {
    ...
    if (!pendingState) {
      ...
      return res
        .status(400)
        .json({ error: 'État "en attente de livraison" non trouvé' });
    }
    ...
    for (const article of articles) {
      ...
      if (!articleData) {
        ...
        return res
          .status(400)
          .json({ error: `Article avec l'ID ${id} non trouvé` });
      }
    ...
    if (
      ...
      paymentMethod.value === "paiement à la livraison"
    ) {
...
const getOrderById = async (req, res) => {
  ...
  if (
    !user ||
    (user.role !== "Administrateur" && user.role !== "fournisseur")
  ) {
    return res
      .status(403)
      .json({
...
const updateOrder = async (req, res) => {
  ...
  try {
    ...
    const changesMessage = getChangesMessage(
      originalOrderData,
      updatedOrderData
    );
...
const assignDeliveryPerson = async (req, res) => {
  ...
  try {
    ...
    if (!deliveryState) {
      ...
      return res
        .status(400)
        .json({ error: 'État "en cours de livraison" non trouvé' });
    }
    const order = await Order.findOne({
      where: {
        id: orderId,
    ...
    const deliveryPerson = await User.findOne({
      where: {
        ...
        deleted: false,
        role_usersID: (
          await RoleUser.findOne({ where: { name: "livreur" } })
        ).id,
      },
    });
    ...
    if (!deliveryPerson) {
      ...
      return res
        .status(400)
        .json({ error: "Livreur non trouvé ou rôle incorrect" });
    }
    await order.update(
      { deliveryID: deliveryPersonId, StatesID: deliveryState.id, adminID },
      { transaction }
    );
    await OrderState.create(
      {
...
const cancelOrder = async (req, res) => {
  ...
  try {
    ...
    const pendingState = await State.findOne({
      where: { value: "En attente de livraison" },
    });
    if (user.role === "fournisseur" && order.StatesID !== pendingState.id) {
      return res
        ...
        .json({
          error:
            "Accès interdit : Vous ne pouvez annuler que les commandes en attente de livraison",
        });
    }

    const canceledState = await State.findOne({
      where: { value: "Commande annulée" },
    });
    if (!canceledState) {
      return res
        .status(500)
        .json({
          error: 'État "Commande annulée" non trouvé dans la base de données',
        });
    }
    ...
    res
      .status(200)
      .json({ message: 'État de la commande mis à jour à "Commande annulée"' });
  } catch (error) {
    console.error(error);
    res
      .status(500)
