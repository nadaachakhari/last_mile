...
const generateDeliveryCode = async () => {
  ...
  const deliveryCode = `BL-${currentYear}-${newNumber
    .toString()
    .padStart(4, "0")}`;
...
const createDeliveryFromOrder = async (req, res) => {
  ...
  try {
    ...
    const existingDeliverySell = await DeliverySell.findOne({
      where: { orderID: orderID },
    });
    ...
    if (!order.deliveryID) {
      return res
        .status(400)
        .json({ message: "Order does not have a valid deliveryID" });
    }
    ...
    const deliveryLinesData = await Promise.all(
    ...
    const total_ht = deliveryLinesData.reduce(
      (acc, line) => acc + line.sale_ht,
      0
    );
    const total_ttc = deliveryLinesData.reduce(
      (acc, line) => acc + line.sale_ttc,
      0
    );
    ...
    await DeliverySellLignes.bulkCreate(
      deliveryLinesData.map((line) => ({
        ...line,
    ...
    const deliveryLinesDetailed = await Promise.all(
      deliveryLinesData.map(async (line) => {
...
const getAllDeliveries = async (req, res) => {
  try {
    const deliveries = await DeliverySell.findAll({
      include: [
        { model: Tiers, as: "customer" },
