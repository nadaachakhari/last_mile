...
const createTier = async (req, res) => {
  const { name, type_tiersID, code, address, postal_code, country, phone, mobile, fax, email, cityID, block, deleted } = req.body;
  try {
    const newTier = await Tiers.create({ name, type_tiersID, code, address, postal_code, country, phone, mobile, fax, email, cityID, block, deleted: true });
...
const updateTier = async (req, res) => {
  ...
  try {
    ...
    await tier.update({ name, type_tiersID, code, address, postal_code, country, phone, mobile, fax, email, cityID, block, deleted });
    res.status(200).json(tier);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
